...
class Router {
  ...
  setDefaultPage(page: View): void {
    this.defaultRoute = { path: '', page }; // page 는 이름 같아서 생략 ( 원래는 page: page )
  }
  addRoutePath(path: string, page: View): void {
    this.routeTable.push({
     // path: path, 이름이 같은경우 생략가능
     // page: page,
     path, page
    });
  }
...
class NewsFeedView extends View{
  ...
  constructor(containerId: string) {
    ...
    if(this.feeds.length === 0){
      this.feeds = store.feeds = this.api.getData();   // 대입연산자를 연속해서 쓰면 맨 오른쪽의 데이터를 왼쪽에 넣고 그 왼쪽 데이터를 다시 자신의 왼쪽에 넣는다는 의미이다.
      this.makeFeeds();
    }
  }
  ...
  render(): void {
    ...
    for (let i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
      const { id, title, comments_count, user, points, time_ago, read } = this.feeds[i];  // 중요!!
      this.addHtml(`
  ...
  private makeFeeds(): void{
    for(let i = 0; i < this.feeds.length; i++){  // 여기서 let i 같은 경우 우리가 봐도, 타입스크립트가 봐도 숫자인게 명확하기 때문에 이런
      this.feeds[i].read = false;                // 경우는 타입스크립트가 내부적으로 타입을 적어준다.( 생략가능 )
    }
  }
}
...
class NewsDetailView extends View{
  ...
  render(): void {
    const id = location.hash.substr(7); //location객체는 브라우저가 기본적으로 제공하는 객체다. 주소와 관련된 다양한 정보를 제공해준다.
    const api = new NewsDetailApi(CONTENT_URL.replace('@id', id));
    const newsDetail: NewsDetail = api.getData(); //replace는 값을 대체해주는 함수(왼쪽에 있는 값을 오른쪽에 있는 값으로 바꾼다)
